...
with open("news/AAPL/20241029/c46d5a41.txt", 'r', encoding='utf-8') as file:
...
content_tokens = tokenizer.tokenize(contents)[:max_length]
contents = tokenizer.convert_tokens_to_string(content_tokens)

# Tokenize the prompt
inputs = tokenizer(prompt, return_tensors="pt").to(device)
input_length = inputs['input_ids'].shape[1]

# Check if input length exceeds model's maximum
if input_length > model.config.max_position_embeddings:
    raise ValueError(f"Input is too long ({input_length} tokens). Max allowed is {model.config.max_position_embeddings} tokens.")

# Generate text
with torch.no_grad():
    output = model.generate(
        **inputs,
        max_length=input_length + 100,  # Generate up to 100 tokens more
