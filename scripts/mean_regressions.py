...
from machine_learning_finance import get_coin_data_frames, attach_markers, generate_probability, \
    ...
    calculate_and_graph_price_probabilities

windows = [300, 600, 900, 1500]

# Parse command line arguments
parser = argparse.ArgumentParser()
parser.add_argument("--data-source", default="ku_coin", choices=["ku_coin", "coin_base", "yf"], help="Data source to use (default: ku_coin)")
...
use_stocks = args.data_source == "yf"
...
if not use_stocks:
    ...
    tickerObj = yf.download(tickers=args.symbol, interval="1d")
    df_raw = pd.DataFrame(tickerObj)
    df_raw = df_raw.tail(args.window)
